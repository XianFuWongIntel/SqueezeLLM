    ...
    def forward(self, x):
        if x.shape[-1] == x.numel():
            ...
            if self.bias is not None:
                ...
                y = torch.zeros((self.outfeatures), device='cpu', dtype=torch.float32)
            ...
            if self.bits == 3:
                ...
                if self.include_sparse and self.topX > 0:
                    quant_cpu.vecquant3matmul_spmv_hybrid_nuq_perchannel(
                        self.rows, 
                        self.cols, 
                        self.vals, 
                        x, 
                    ...
                    quant_cpu.vecquant3matmul_spmv_nuq_perchannel(
                        self.rows, 
                        self.cols, 
                        self.vals, 
                        x, 
                    ...
                    quant_cpu.vecquant3matmul_nuq_perchannel(
                        x, 
                        self.qweight, 
                        y, 
                        self.lookup_table,
                ...
                x = x.float()
                if self.include_sparse and self.topX > 0:
                    quant_cpu.vecquant4matmul_spmv_hybrid_nuq_perchannel(
                        self.rows, 
                        self.cols, 
                        self.vals, 
                        x, 
                    ...
                    quant_cpu.vecquant4matmul_spmv_nuq_perchannel(
                        self.rows, 
                        self.cols, 
                        self.vals, 
                        x, 
                    ...
                    quant_cpu.vecquant4matmul_nuq_perchannel(x, self.qweight, y, self.lookup_table)
            y = y.to(dtype)
            return y.reshape(outshape)
        else:
            out_shape = x.shape[:-1] + (self.outfeatures, )
            x = x.reshape(-1,x.shape[-1])
            out = torch.zeros((x.shape[0], self.outfeatures), device='cpu', dtype=torch.float32)
            dtype = x.dtype
            if self.bits == 3:
                x = x.float()
                if self.include_sparse and self.topX > 0:
                    quant_cpu.vecquant3matmul_spmv_hybrid_nuq_perchannel_batched(
                        self.rows, 
                        self.cols, 
                        self.vals, 
                        x, 
                    ...
                    quant_cpu.vecquant3matmul_spmv_nuq_perchannel_batched(
                        self.rows, 
                        self.cols, 
                        self.vals, 
                        x, 
                    ...
                    quant_cpu.vecquant3matmul_nuq_perchannel_batched(
                        x, 
                        self.qweight, 
                        out, 
                        self.lookup_table,
                ...
                x = x.float()
                if self.include_sparse and self.topX > 0:
                    quant_cpu.vecquant4matmul_spmv_hybrid_nuq_perchannel_batched(
                        self.rows, 
                        self.cols, 
                        self.vals, 
                        x, 
                    ...
                    quant_cpu.vecquant4matmul_spmv_nuq_perchannel_batched(
                        self.rows, 
                        self.cols, 
                        self.vals, 
                        x, 
                    ...
                    quant_cpu.vecquant4matmul_nuq_perchannel_batched(
                        x, 
                        self.qweight, 
                        out, 
                        self.lookup_table,
